...
title Monero Miner Setup - Low-End PC
color 0A

REM ====================================================================
REM Ultimate Monero Miner - Low-End Edition (Fixed JSON Config)
REM Optimized for dual-core PCs with limited resources
...
echo ===============================
echo Starting Monero Miner on Low-End PC
echo ===============================
...
if not defined AFFINITY_MASK (
    set "AFFINITY_MASK=3"  REM Fallback for 2 cores
)
...
(
  ...
  echo start "" "%DEST%\xmrig.exe" --config="%DEST%\config.json" ^
...
echo [✔] Miner installed (optimized for low-end PC)
